  ...
  : _count_probability()
{
  ...
  /** BELOW IS EWMA **/
  /* step 1: make fake component processes (will not be used in EWMA) */
  std::vector< bool > components( num_ticks, true );

  /* step 2: make the tick forecast - Not needed in EWMA as we are not interested in forecasting */
  /* ProcessForecastTick tick_forecast( tick_time, example, tick_upper_limit );*/

  /* step 3: Fill it with zeros */
